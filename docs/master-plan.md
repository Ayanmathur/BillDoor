...
- **resources** — `client_id`, `name`, `active`.
...
- **catalog_items** — `client_id`, `name`, `type` (product/service), `price`, `unit`, `default_gst_percent`, `default_resource_id`, `default_duration_min`, **`barcode_value`** (new, nullable, unique per `client_id`), **`barcode_format`** (new, default `'code128'`), **`barcode_auto_generated`** (new, bool) — see §5.
- **bills** — `client_id`, `customer_id`, `bill_number`, `bill_slug`, `items` (jsonb — each line now optionally carries `added_via: 'search' | 'barcode'` for analytics, see §5), `subtotal`, `discount`, `gst`, `extra_charges`, `grand_total`, `whatsapp_sent`, `sent_via`, `sent_at`, `created_at`.
...
| 3 | P1 | Billit (core billing, no barcode yet) | Second most requested vertical need. |
...
| 5 | P1 | Appointer | Needed for clinic/salon vertical. |
